...
impl Command for ChromeCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &CommandContext) -> CommandResult {
        ...
        let content = match arg.as_str() {
            "install" | "install-extension" => format!(
            ...
            "permissions" | "manage-permissions" => format!(
            ...
            "toggle" => "Claude in Chrome default behavior toggled.\n\n\
            ...
            other => format!(
                "Unknown subcommand: \"{other}\". Use install, permissions, reconnect, toggle, or status."
            ),
        };
