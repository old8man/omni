...
fn build_sections(ctx: &StatusDialogContext) -> Vec<Section> {
    ...
    // ── System info ──────────────────────────────────────────────────────────
    {
        ...
        // Sessions dir
        if let Ok(dir) = claude_core::config::paths::sessions_dir() {
            rows.push(StatusRow::colored(
                "Sessions dir",
                dir.display().to_string(),
                Color::DarkGray,
    ...
    // ── Authentication ───────────────────────────────────────────────────────
    {
        ...
        match claude_core::auth::profiles::get_active_profile() {
            Some(profile) => {
                rows.push(StatusRow::colored(
                    "Logged in as",
                    profile.email.clone(),
        ...
        // List all profiles
        let profiles = claude_core::auth::profiles::list_profiles();
        if !profiles.is_empty() {
            let active_name = claude_core::auth::profiles::get_active_profile_name();
            for p in &profiles {
    ...
    // ── Configuration ────────────────────────────────────────────────────────
    {
        ...
        if let Ok(settings_path) = claude_core::config::paths::user_settings_path() {
...
fn fmt_num(n: u64) -> String {
    claude_core::utils::format::format_tokens(n)
}
