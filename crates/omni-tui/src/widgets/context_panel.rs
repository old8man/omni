...
impl Default for ContextPanel {
    fn default() -> Self {
        Self::new(omni_core::utils::context::MODEL_CONTEXT_WINDOW_DEFAULT)
    }
}
...
impl ContextPanel {
    ...
    pub fn configure_for_model(&mut self, model: &str) {
        self.max_tokens = omni_core::utils::context::get_context_window_for_model(model);
    }
}
...
///
/// Delegates to [`omni_core::utils::format::format_tokens`].
fn format_tokens(tokens: u64) -> String {
    omni_core::utils::format::format_tokens(tokens)
}

/// Truncate a string in the middle, adding "..." if needed.
///
/// Delegates to [`omni_core::utils::format::truncate_middle`].
fn truncate(s: &str, max_len: usize) -> String {
    omni_core::utils::format::truncate_middle(s, max_len)
}
