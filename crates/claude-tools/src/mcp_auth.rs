...
impl ToolExecutor for McpAuthTool {
    ...
    async fn call(
        ...
        tokio::spawn(async move {
            ...
            match claude_core::mcp::auth::exchange_code_for_tokens(
                &metadata,
                &client_id,
                None,
                &code,
            ...
            .await
            {
                Ok(tokens) => {
                    ...
                    let server_key = claude_core::mcp::auth::get_server_key(
                        &server_name,
                        config.config.url.as_deref().unwrap_or(""),
                    );
                    let mut store = claude_core::mcp::auth::OAuthStore::load();
                    store.set(
                        server_key.clone(),
                        claude_core::mcp::auth::StoredServerOAuth {
