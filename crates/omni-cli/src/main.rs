...
async fn main() -> Result<()> {
    ...
    match &cli.command {
        Some(SubCommand::Login) => {
            ...
            if let Some(existing) = omni_core::auth::storage::load_tokens().await? {
                if !omni_core::auth::pkce::is_token_expired(existing.expires_at) {
                    eprintln!();
            ...
            let result = omni_core::auth::pkce::run_oauth_login(true).await?;

            // Store tokens securely
            omni_core::auth::storage::store_tokens(&result.tokens).await?;

            eprintln!("\x1b[32m  Login successful!\x1b[0m");
            eprintln!();
            if omni_core::auth::oauth_config::has_inference_scope(&result.tokens.scopes) {
        ...
        Some(SubCommand::Logout) => {
            ...
            omni_core::auth::storage::delete_tokens().await?;
        ...
        Some(SubCommand::Config) => {
            let cwd = cli
                ...
                .map(Ok)
                .unwrap_or_else(std::env::current_dir)?;
            let root = omni_core::config::paths::detect_project_root(&cwd);
            println!("Project root: {}", root.display());
            println!(
                "Config dir: {}",
                omni_core::config::paths::claude_dir()?.display()
            );
            return Ok(());
        }
        None => {}
    ...
    let permission_mode = if cli.dangerously_skip_permissions {
        omni_core::permissions::types::PermissionMode::Bypass
    } else {
        omni_core::permissions::types::PermissionMode::Default
    };

    let bootstrap_config = omni_core::bootstrap::BootstrapConfig {
    ...
    let bootstrap_result = omni_core::bootstrap::initialize(bootstrap_config).await?;
    let state_store = bootstrap_result.state;

    // Resolve auth from bootstrap result
    use omni_core::auth::resolve::AuthResolution;
    ...
    let auth = match auth_resolution {
        ...
        AuthResolution::OAuthProxy => {
            omni_core::api::client::AuthMethod::ApiKey("proxy".into())
        }
        AuthResolution::None => {
            eprintln!();
            eprintln!("  \x1b[1mWelcome to Claude Code!\x1b[0m");
            eprintln!();
            if omni_core::api::claude_proxy::is_claude_available() {
    ...
    let session_memory_state = omni_core::services::SessionMemoryState::new();
    ...
    let assistant_state = {
        ...
        if state_store.is_assistant_mode() {
            omni_core::assistant::activate_kairos(&mut state);
        }
        state
    };

    // Build tool registry
    let tools = omni_tools::build_default_registry();
    ...
    let system_prompt_values = omni_core::context::system_prompt::build_system_prompt_full(
        &project_root,
        &model,
        &tool_names,
        memory_prompt,
    ...
    let system_prompt: Vec<omni_core::types::content::ContentBlock> = system_prompt_values
        .into_iter()
        .filter_map(|v| {
            v.get("text").and_then(|t| t.as_str()).map(|text| {
                omni_core::types::content::ContentBlock::Text {
                    text: text.to_string(),
                }
            })
        })
    ...
    let api_config = omni_core::api::client::ApiConfig {
        model,
    ...
    let api_client = omni_core::api::client::ApiClient::new(api_config, auth);
    ...
    // Create query engine
    let mut query_engine = omni_core::query::engine::QueryEngine::new(
        api_client,
        system_prompt,
        tool_defs,
        cancel.clone(),
    ...
    if let Some(prompt) = cli.prompt {
        ...
        if use_proxy {
            ...
            omni_core::api::claude_proxy::stream_via_claude(
                &prompt,
                model_opt,
                cancel.clone(),
                |text| print!("{}", text),
        ...
        use omni_core::permissions::evaluator::evaluate_permission_sync;
        use omni_core::permissions::types::{PermissionBehavior, ToolPermissionContext};
        use omni_core::query::engine::TurnResult;
        use omni_core::types::events::StreamEvent;
        use omni_tools::ToolUseContext;
        use tokio::sync::mpsc;
        use std::sync::Arc;
        ...
        {
            let tool_map: Arc<std::collections::HashMap<String, Arc<dyn omni_tools::ToolExecutor>>> =
                Arc::new(
                    tools
                        .all()
                        .into_iter()
            ...
            let tool_call_fn: omni_core::query::tool_executor::ToolCallFn = Arc::new(
                move |name: String,
                      _id: String,
        ...
        loop {
            ...
            match result {
                TurnResult::Done(_) => {
                    let token_count = omni_core::services::rough_token_count_estimation(
