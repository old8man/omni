        ...
        if run_in_background {
            ...
            if let Some(tm) = task_manager {
                let task_id = {
                    ...
                    let params = omni_core::tasks::types::CreateTaskParams {
                        subject: description.clone(),
                        description: description.clone(),
                        task_type: omni_core::tasks::types::TaskType::LocalBash,
                        active_form: None,
                        owner: None,
                        metadata,
                    };
                ...
                tokio::spawn(async move {
                    ...
                    if let Ok(output) = output {
                        ...
                        let status = if code == 0 {
                            omni_core::tasks::types::TaskStatus::Completed
                        } else {
                            omni_core::tasks::types::TaskStatus::Failed
                        };
                        let _ = tm.update(&task_id_clone, omni_core::tasks::types::UpdateTaskParams {
                            status: Some(status),
                            ..Default::default()
                        }).await;
                    }
