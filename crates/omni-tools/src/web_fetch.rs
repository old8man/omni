...
impl ToolExecutor for WebFetchTool {
    ...
    async fn call(
        ...
        if bytes.len() > MAX_RESPONSE_BYTES {
            return Ok(ToolResultData {
                data: json!({
                    "error": format!("Response too large: {} (max {})", omni_core::utils::format::format_file_size(bytes.len() as u64), omni_core::utils::format::format_file_size(MAX_RESPONSE_BYTES as u64)),
                    "url": url,
                }),
                is_error: true,
            });
